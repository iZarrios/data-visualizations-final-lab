...
def build_figure() -> go.Figure:
    ...
    driver_heat = (
        driver_wins.groupby(["surname", "decade"]).size().reset_index(name="wins")
    )
    driver_pivot = driver_heat.pivot(
        index="surname", columns="decade", values="wins"
    ).fillna(0)
    driver_pivot = driver_pivot.loc[
        driver_pivot.sum(axis=1).sort_values(ascending=False).index
    ]

    constructor_wins = wins.merge(
        load_constructors()[["constructorId", "name"]], on="constructorId"
    )
    constructor_heat = (
        constructor_wins.groupby(["name", "decade"]).size().reset_index(name="wins")
    )
    constructor_pivot = constructor_heat.pivot(
        index="name", columns="decade", values="wins"
    ).fillna(0)
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=(
            "Driver Dominance per Decade",
            "Constructor Dominance per Decade",
        ),
