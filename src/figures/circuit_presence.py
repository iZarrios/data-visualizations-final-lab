...
def build_figure() -> go.Figure:
    ...
    if matrix_pivot.empty:
        fig = go.Figure()
        apply_theme(fig, title="Classic F1 Circuits: Historical Presence Matrix", height=600)
        return fig
    ...
    apply_theme(
        ...
        height=700,
        showlegend=False,
        xaxis=dict(title="<b>Season</b>", tickangle=-45, dtick=2, gridcolor=COLORS["grid_line"]),
    ...
    fig.add_annotation(
        x=1.06, y=0.57, xref="paper", yref="paper",
        text="<b>Not Hosted</b>", showarrow=False,
        font=dict(size=11, color=COLORS["text_dark"]), xanchor="left", yanchor="middle",
    )
    fig.add_shape(
        type="rect", x0=1.02, x1=1.045, y0=0.555, y1=0.585,
        xref="paper", yref="paper", fillcolor=COLORS["white"],
        line=dict(color=COLORS["text_dark"], width=1.5), layer="above",
    )
    fig.add_annotation(
        x=1.06, y=0.48, xref="paper", yref="paper",
        text="<b>Hosted</b>", showarrow=False,
        font=dict(size=11, color=COLORS["text_dark"]), xanchor="left", yanchor="middle",
    )
    fig.add_shape(
        type="rect", x0=1.02, x1=1.045, y0=0.465, y1=0.495,
        xref="paper", yref="paper", fillcolor=COLORS["hosted_blue"],
        line=dict(color=COLORS["text_dark"], width=1.5), layer="above",
    )
    return fig
